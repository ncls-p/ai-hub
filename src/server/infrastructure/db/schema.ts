...
export const roleBindings = pgTable(
    ...
    (t) => ({
        principalRoleResource: index(
            "role_bindings_principal_role_resource",
        ).on(t.principalType, t.principalId, t.resourceType, t.resourceId),
    }),
);
...
export const auditEvents = pgTable(
    ...
    (t) => ({
        ...
        resource: index("audit_events_resource").on(
            t.resourceType,
            t.resourceId,
        ),
...
export const agents = pgTable(
    "agents",
    {
        ...
        slug: varchar("slug", { length: 128 }).notNull(),
        description: text("description"),
        visibility: agentVisibilityEnum("visibility")
            .notNull()
            .default("private"),
        sourceType: agentSourceTypeEnum("source_type")
            .notNull()
            .default("custom"),
...
export const agentToolBindings = pgTable(
    ...
    (t) => ({
        agentVersionTool: uniqueIndex(
            "agent_tool_bindings_version_tool_unique",
        ).on(t.agentVersionId, t.toolSource, t.toolId),
    }),
);
...
export const toolInvocations = pgTable(
    ...
    (t) => ({
        ...
        conversation: index("tool_invocations_conversation").on(
            t.conversationId,
        ),
    }),
);
...
export const documentChunks = pgTable(
    ...
    (t) => ({
        document: index("document_chunks_document").on(
            t.documentId,
            t.chunkIndex,
        ),
    }),
