...
export async function updateModel(modelId: string, input: UpdateModelInput) {
    ...
    if (input.contextWindow !== undefined)
        updates.contextWindow = input.contextWindow || null;
    if (input.maxOutputTokens !== undefined)
        updates.maxOutputTokens = input.maxOutputTokens || null;
    if (input.inputTokenCost !== undefined)
        updates.inputTokenCost = input.inputTokenCost || null;
    if (input.outputTokenCost !== undefined)
        updates.outputTokenCost = input.outputTokenCost || null;
...
export async function discoverModels(
    ...
    if (!adapter.listModels) {
        throw new Error(
            `Model discovery not supported for kind: ${provider.kind}`,
        );
    }
